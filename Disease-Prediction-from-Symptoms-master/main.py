...
class DiseasePrediction:
    ...
    def __init__(self, model_name=None):
        ...
        try:
            ...
            # To prevent further errors if config loading fails
            self.config = {}

        # Verbose flag (default to False if missing)
    ...
    def _load_train_dataset(self):
        ...
        # The training data columns, excluding last two columns which presumably are 'prognosis' and something else
        cols = df_train.columns[:-2]
        train_features = df_train[cols]
        train_labels = df_train['prognosis']

        # Data sanity check
        assert (len(train_features.iloc[0]) == 132), "Train features count mismatch"
        assert (len(train_labels) == train_features.shape[0]), "Train labels count mismatch"
    ...
    def _load_test_dataset(self):
        ...
        # For test data, exclude the last column which is 'prognosis'
        cols = df_test.columns[:-1]
        test_features = df_test[cols]
        test_labels = df_test['prognosis']

        # Data sanity check
        assert (len(test_features.iloc[0]) == 132), "Test features count mismatch"
        assert (len(test_labels) == test_features.shape[0]), "Test labels count mismatch"
    ...
    # Feature Correlation (only numeric columns)
    def _feature_correlation(self, data_frame=None, show_fig=False):
        # Select only numeric columns to avoid errors with strings in prognosis column
    ...
    # Model Selection based on model_name
